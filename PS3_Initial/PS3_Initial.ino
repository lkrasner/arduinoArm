...
void setup()  //sets up servos and starts serial communication proccess
{
  ...
  if (Usb.Init() == -1) { //does some usb set up thing.  important.
    Serial.print(F("\r\nOSC did not start"));
    while(1); //halt
  }
...
void loop() //calls all the other functions in the proper order
{
  ...
  Usb.Task(); //no idea what it is, doesn't work without it, so don't touch it.
...
double calculateSpeed(int currentAnalog, int pos) //calculates the speed using a power curve that seems to feel nice.
{
  double a;
